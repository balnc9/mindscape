...
function info_disk {
    $disks = Get-PSDrive -PSProvider FileSystem | Where-Object { $_.Name -in $ShowDisks -or $ShowDisks -eq "*" }

    $lines = foreach ($d in $disks) {
        $used = ($d.Used / 1GB)
        $total = ($d.Used + $d.Free) / 1GB
        $percent = ($used / $total) * 100

        "{0}: {1:N1} GB / {2:N1} GB (`e[32m{3:N0}%`e[0m)" -f $d.Name, $used, $total, $percent
    }
